...
def minimax(board,depth,alpha,beta,chip) :
    ...
    if chip == 'a' :
        ...
        for col,next_board in allNextMove :
            ...
            if beta <= alpha :
                break
        for k_score,k_col in temp_score :
            if k_score == max_score :
                # add candidate col
                candidate_next_col.append(k_col)
        candidate_next_col.sort(key=cmp_function)
        next_col = candidate_next_col[0]
